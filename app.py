...
def reset_with_token(token):
    ...
    try:
        email = s.loads(token, salt='password-reset-salt', max_age=3600)  # 60 min after code verification
...
def admin_forgot_password():
    if request.method == "POST":
        ...
        flash("If an admin account with that email exists, a 6-digit verification code has been sent. It is valid for 1 hour.", "success")
